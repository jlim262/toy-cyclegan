...
class Discriminator(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        self.conv1 = conv(in_channels, out_channels,
                          kernel_size=4, stride=2, padding=1, norm=False)
        self.conv2 = conv(out_channels, out_channels*2,
                          kernel_size=4, stride=2, padding=1)
        self.conv3 = conv(out_channels*2, out_channels*4,
                          kernel_size=4, stride=2, padding=1)
        self.conv4 = conv(out_channels*4, out_channels*8,
                          kernel_size=4, stride=1, padding=1)
        self.conv5 = conv(out_channels*8, 1, kernel_size=4,
                          stride=1, padding=1)
...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels=64, use_dropout=False):
        ...
        self.conv1 = conv(in_channels, out_channels,
                          7, 1, 0)               # conv
        self.conv2 = conv(out_channels, out_channels*2, 3,
                          2, 1)            # downsampling
        self.conv3 = conv(out_channels*2, out_channels*4,
                          3, 2, 1)          # downsampling
        # resnet blocks, 6 when the size of input image is 128 x 128
        self.residuals = residual(out_channels*4, use_dropout, n_blocks=6)
        self.deconv1 = deconv(out_channels*4, out_channels *
                              2, 3, 2, 1, 1)   # upsampling
        self.deconv2 = deconv(out_channels*2, out_channels,
                              3, 2, 1, 1)     # upsampling
        self.conv4 = conv(out_channels, 3, 7, 1,
                          0)                         # conv
