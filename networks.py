...
class ResidualBlock(nn.Module):
    ...
    def conv(self, dim, use_dropout):
        sequence = [
            ...
            nn.Conv2d(dim, dim, kernel_size=3, padding=0, bias=True), nn.InstanceNorm2d(dim), 
            nn.ReLU(True)]
...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels=64, use_dropout=False):
        ...
        self.residuals = residual(out_channels*4, use_dropout, n_blocks=6)  # resnet blocks, 6 when the size of input image is 128 x 128
